...
- 签到工具：课程列表、数字签到码查询、签到提醒、自动监听；支持手动确认，或切换全自动后免确认提交数字 / 雷达签到。
- 雷达点名：校区 / 教学楼预设、默认位置、失败诊断、坐标微调和自定义位置库；自动模式下可三点测距推算目标坐标后重提。
...
进入 `教学平台` 登录并打开课程列表后，可使用页面上方的签到监听：

- `手动确认`：检测到数字 / 雷达签到时弹窗确认再提交。
- `全自动`：检测到数字 / 雷达签到后直接提交，不再弹确认框。
- 自动雷达签到会先使用设置页中的 `默认雷达签到位置`；若平台返回距离偏差，会再提交两个附近探测点，根据三点距离推算目标坐标后重试。
- 未设置默认雷达位置时，自动雷达会回退到手动确认。

雷达页面仍支持手动选预设、自定义坐标和现场微调，适合需要人工核对位置时使用。
